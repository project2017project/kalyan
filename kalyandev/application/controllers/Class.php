...
class Uclass extends MY_Controller {
	...
	public function index()
	{
		...
		$data['page_uclass'] = $this->Model_common->all_page_uclass();
		$data['comment'] = $this->Model_common->all_comment();
		$data['social'] = $this->Model_common->all_social();
		$data['all_news'] = $this->Model_common->all_news();

		$data['uclass'] = $this->Model_uclass->all_uclass();
		$data['portfolio_footer'] = $this->Model_portfolio->get_portfolio_data();

		$data['check_captcha'] = $this->Model_uclass->check_captcha();
		$data['total_captcha'] = $this->Model_uclass->total_captcha();

		$this->load->view('view_header',$data);
		$this->load->view('view_uclass',$data);
	...
	public function view($id=0)
	{
		...
		$data['uclass'] = $this->Model_uclass->all_uclass();
		$data['uclass'] = $this->Model_uclass->uclass_detail($id);
		...
		$this->load->view('view_header',$data);
		$this->load->view('view_uclass_detail',$data);
	...
	public function send_email() 
	{
		...
		$data['setting'] = $this->Model_common->all_setting();
		$check_captcha = $this->Model_uclass->check_captcha();

		$error = '';

		if(isset($_POST['form_uclass'])) {
            ...
            if($check_captcha['captcha_uclass_detail'] == 'Show')
		    {
			    ...
			    {
			    	$aa = $this->Model_uclass->get_particular_captcha($r_serial);
		    ...
		    if($valid == 1)
		    {
				...
				<html><head><title>Email Sending</title></head><body>
					...
					<b>Class Name: </b> '.$_POST['uclass'].'<br><br>
				...
				$this->email->subject('Class Page Email');
				$this->email->message($msg);

				$this->email->send();

		        $success = SUCCESS_UCLASS_PAGE_FORM;
